<x-layout>
    <x-slot name="head">
        <title>
            {{ __('Edit New Company Profile Video') }} | {{ env('APP_NAME') }}
        </title>
    </x-slot>
    ...
    <div class="content">
        ...
        <div class="mt-5 grid grid-cols-12 gap-6">
            <div class="intro-y col-span-12 lg:col-span-6">
                <!-- BEGIN: Form Layout -->
                <form action="{{ route('company-profile-videos.update', $company_profile_video->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="intro-y box p-5">
                        <div>
                            ...
                            <input class="form-control {{ $errors->has('name') ? 'border-danger' : '' }} w-full"
                                ...
                                value="{{ old('name', $company_profile_video->name) }}">
                            @error('name')
                        ...
                        <div class="mt-3">
                            ...
                            <input class="form-control {{ $errors->has('link_video') ? 'border-danger' : '' }} w-full"
                                ...
                                name="link_video" value="{{ old('link_video', $company_profile_video->link_video) }}">
                            @error('link_video')
                                <div class="pristine-error mt-2 text-danger">{{ $message }}</div>
                            @enderror
                            <div class="form-help">
