<x-layout>
    ...
    <!-- BEGIN: Content -->
    <div class="content">
        <h2 class="intro-y mt-10 text-lg font-medium">{{ __('Post Types List') }}</h2>
        <div class="mt-5 grid grid-cols-12 gap-6">
            <div class="intro-y col-span-12 mt-2 flex flex-wrap items-center sm:flex-nowrap">
                <a class="btn btn-primary mr-2 shadow-md" href="{{ route('post-types.create') }}">
                    {{ __('Add New Post Type') }}
                </a>
                {{-- <div class="dropdown">
                ...
                @if ($post_types->hasPages())
                    {{ $post_types->links('pagination.info') }}
                @else
                            ...
                            <tr class="intro-x">
                                <td>
                                    ...
                                    <div class="mt-0.5 whitespace-nowrap text-xs text-slate-500">
                                        NO. {{ $post_types->firstItem() + $loop->index }}
                                    </div>
                                </td>
                                <td class="table-report__action w-56">
                                    <div class="flex items-center justify-center">
                                        <a class="mr-3 flex items-center"
                                            href="{{ route('post-types.edit', $post_type->id) }}">
                                            <i class="mr-1 h-4 w-4" data-lucide="check-square"></i> {{ __('Edit') }}
                                        </a>
                                        <a class="flex items-center text-danger" href="javascript:;"
                                            data-tw-toggle="modal"
                                            data-tw-target="#delete-confirmation-modal-{{ $post_type->id }}">
            ...
            <div class="intro-y col-span-12 flex flex-wrap items-center sm:flex-row sm:flex-nowrap">
                {{ $post_types->links('pagination.page') }}
        ...
        @forelse ($post_types as $post_type)
            <div class="modal" id="delete-confirmation-modal-{{ $post_type->id }}" tabindex="-1" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-body p-0">
                            <form action="{{ route('post-types.destroy', $post_type->id) }}" method="POST">
                                @csrf
                                @method('delete')
