                ...
                <form action="{{ route('features.update', $feature->id) }}" method="POST">
                    ...
                    <div class="intro-y box p-5">
                        ...
                        <div class="mt-3">
                            ...
                            <div class="mt-2">
                                <textarea class="text-area {{ $errors->has('description') ? '!border-danger' : '' }}" id="description" rows="5"
                                    name="description" placeholder="{{ __('Add description') }}">{{ old('description', $feature->description) }}</textarea>
                            </div>
                            @error('description')
                                <div class="pristine-error mt-2 text-danger">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="mt-5 text-right">
                            <a class="btn btn-outline-secondary mr-1 w-24" href="{{ route('features.index') }}">
                                {{ __('Cancel') }}
                            </a>
