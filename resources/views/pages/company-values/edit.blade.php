<x-layout>
    <x-slot name="head">
        <title>
            {{ __('Edit Company Value') }} | {{ env('APP_NAME') }}
        </title>
    </x-slot>
    ...
    <div class="content">
        <div class="intro-y mt-8 flex items-center">
            <h2 class="mr-auto text-lg font-medium">
                {{ __('Edit Company Value') }}
            </h2>
        </div>
        <div class="mt-5 grid grid-cols-12 gap-6">
            <div class="intro-y col-span-12 lg:col-span-6">
                <!-- BEGIN: Form Layout -->
                <form action="{{ route('company-values.update', $company_value->id) }}" method="POST"
                    ...
                    <div class="intro-y box p-5">
                        <div>
                            ...
                            <input class="form-control {{ $errors->has('name') ? 'border-danger' : '' }} w-full"
                                ...
                                value="{{ old('name', $company_value->name) }}">
                            @error('name')
