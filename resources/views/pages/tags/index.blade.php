<x-layout>
    ...
    <!-- BEGIN: Content -->
    <div class="content">
        <h2 class="intro-y mt-10 text-lg font-medium">{{ __('Tags List') }}</h2>
        <div class="mt-5 grid grid-cols-12 gap-6">
            <div class="intro-y col-span-12 mt-2 flex flex-wrap items-center sm:flex-nowrap">
                <a class="btn btn-primary mr-2 shadow-md" href="{{ route('tags.create') }}">
                    {{ __('Add New Tag') }}
                </a>
                {{-- <div class="dropdown">
                            ...
                            <tr class="intro-x">
                                <td>
                                    ...
                                    <div class="mt-0.5 whitespace-nowrap text-xs text-slate-500">
                                        NO. {{ $tags->firstItem() + $loop->index }}
                                    </div>
                                </td>
                                <td class="table-report__action w-56">
                                    <div class="flex items-center justify-center">
                                        <a class="mr-3 flex items-center" href="{{ route('tags.edit', $tag->id) }}">
                                            <i class="mr-1 h-4 w-4" data-lucide="check-square"></i> {{ __('Edit') }}
                                        </a>
                                        <a class="flex items-center text-danger" href="javascript:;"
                                            data-tw-toggle="modal"
                                            data-tw-target="#delete-confirmation-modal-{{ $tag->id }}">
        ...
        @forelse ($tags as $tag)
            <div class="modal" id="delete-confirmation-modal-{{ $tag->id }}" tabindex="-1" aria-hidden="true">
