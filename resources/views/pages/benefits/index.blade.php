<x-layout>
    ...
    <!-- BEGIN: Content -->
    <div class="content">
        <h2 class="intro-y mt-10 text-lg font-medium">{{ __('Benefits List') }}</h2>
        <div class="mt-5 grid grid-cols-12 gap-6">
            <div class="intro-y col-span-12 mt-2 flex flex-wrap items-center sm:flex-nowrap">
                <a class="btn btn-primary mr-2 shadow-md" href="{{ route('benefits.create') }}">
                    {{ __('Add New Benefit') }}
                </a>
                {{-- <div class="dropdown">
                ...
                {{ $benefits->links('pagination.info') }}
            ...
            <div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
                <table class="table-report -mt-2 table">
                    <thead>
                        <tr>
                            <th class="whitespace-nowrap">{{ __('ICON') }}</th>
                            ...
                            <tr class="intro-x">
                                <td class="w-20">
                                    <div class="flex">
                                        <div class="image-fit zoom-in h-10 w-10">
                                            <img class="tooltip rounded-full" alt="{{ $benefit->name }}"
                                                src="{{ Str::contains($benefit->icon, 'placeholder') ? $benefit->icon : asset($benefit->getIcon()) }}"
                                                title="{{ $benefit->name }}">
                                        </div>
                                    </div>
                                </td>
                                <td>
                                    ...
                                    <div class="mt-0.5 whitespace-nowrap text-xs text-slate-500">
                                        NO. {{ $benefits->firstItem() + $loop->index }}
                                    </div>
                                </td>
                                ...
                                <td class="table-report__action w-56">
                                    <div class="flex items-center justify-center">
                                        ...
                                        <a class="flex items-center text-danger" href="javascript:;"
                                            ...
                                            data-tw-target="#delete-confirmation-modal-{{ $benefit->id }}">
            ...
            <div class="intro-y col-span-12 flex flex-wrap items-center sm:flex-row sm:flex-nowrap">
                {{ $benefits->links('pagination.page') }}
        ...
        @forelse ($benefits as $benefit)
            <div class="modal" id="delete-confirmation-modal-{{ $benefit->id }}" tabindex="-1" aria-hidden="true">
