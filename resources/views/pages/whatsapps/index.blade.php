<x-layout>
    ...
    <!-- BEGIN: Content -->
    <div class="content">
        <h2 class="intro-y mt-10 text-lg font-medium">{{ __('Whatsapps List') }}</h2>
        <div class="mt-5 grid grid-cols-12 gap-6">
            <div class="intro-y col-span-12 mt-2 flex flex-wrap items-center sm:flex-nowrap">
                <a class="btn btn-primary mr-2 shadow-md" href="{{ route('whatsapps.create') }}">
                    {{ __('Add New Whatsapp') }}
                </a>
                {{-- <div class="dropdown">
                ...
                {{ $whatsapps->links('pagination.info') }}
            ...
            <div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
                <table class="table-report -mt-2 table">
                    <thead>
                        <tr>
                            <th class="whitespace-nowrap">{{ __('WHATSAPP') }}</th>
                            ...
                            <tr class="intro-x">
                                <td>
                                    <a class="whitespace-nowrap font-medium" href="">
                                        {{ $whatsapp->country->dial_code }}{{ $whatsapp->number }}
                                    </a>
                                    <div class="mt-0.5 whitespace-nowrap text-xs text-slate-500">
                                        NO. {{ $whatsapps->firstItem() + $loop->index }}
                                    </div>
                                </td>
                                <td class="w-40">{{ $whatsapp->is_main }}</td>
                                <td class="w-80">{{ $whatsapp->country->name }}</td>
                                <td class="table-report__action w-56">
                                    <div class="flex items-center justify-center">
                                        <a class="mr-3 flex items-center"
                                            href="{{ route('whatsapps.edit', $whatsapp->id) }}">
                                            <i class="mr-1 h-4 w-4" data-lucide="check-square"></i> {{ __('Edit') }}
                                        </a>
                                        <a class="flex items-center text-danger" href="javascript:;"
                                            data-tw-toggle="modal"
                                            data-tw-target="#delete-confirmation-modal-{{ $whatsapp->id }}">
            ...
            <div class="intro-y col-span-12 flex flex-wrap items-center sm:flex-row sm:flex-nowrap">
                {{ $whatsapps->links('pagination.page') }}
        ...
        @forelse ($whatsapps as $whatsapp)
            <div class="modal" id="delete-confirmation-modal-{{ $whatsapp->id }}" tabindex="-1" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-body p-0">
                            <form action="{{ route('whatsapps.destroy', $whatsapp->id) }}" method="POST">
                                @csrf
                                @method('delete')
