<x-layout>
    ...
    <!-- BEGIN: Content -->
    <div class="content">
        <h2 class="intro-y mt-10 text-lg font-medium">{{ __('Phone Numbers List') }}</h2>
        <div class="mt-5 grid grid-cols-12 gap-6">
            <div class="intro-y col-span-12 mt-2 flex flex-wrap items-center sm:flex-nowrap">
                <a class="btn btn-primary mr-2 shadow-md" href="{{ route('phone-numbers.create') }}">
                ...
                {{ $phone_numbers->links('pagination.info') }}
            ...
            <div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
                <table class="table-report -mt-2 table">
                    <thead>
                        <tr>
                            <th class="whitespace-nowrap">{{ __('PHONE NUMBER') }}</th>
                            <th class="whitespace-nowrap">{{ __('TYPE') }}</th>
                            <th class="whitespace-nowrap">{{ __('COUNTRY') }}</th>
                            ...
                            <tr class="intro-x">
                                <td>
                                    <a class="whitespace-nowrap font-medium" href="">
                                        {{ $phone_number->country->dial_code }}{{ $phone_number->number }}
                                    </a>
                                    <div class="mt-0.5 whitespace-nowrap text-xs text-slate-500">
                                        NO. {{ $phone_numbers->firstItem() + $loop->index }}
                                    </div>
                                </td>
                                <td class="w-40">{{ $phone_number->is_main }}</td>
                                <td class="w-80">{{ $phone_number->country->name }}</td>
                                <td class="table-report__action w-56">
                                    <div class="flex items-center justify-center">
                                        <a class="mr-3 flex items-center"
                                            href="{{ route('phone-numbers.edit', $phone_number->id) }}">
                                            <i class="mr-1 h-4 w-4" data-lucide="check-square"></i> {{ __('Edit') }}
                                        </a>
                                        <a class="flex items-center text-danger" href="javascript:;"
                                            data-tw-toggle="modal"
                                            data-tw-target="#delete-confirmation-modal-{{ $phone_number->id }}">
            ...
            <div class="intro-y col-span-12 flex flex-wrap items-center sm:flex-row sm:flex-nowrap">
                {{ $phone_numbers->links('pagination.page') }}
        ...
        @forelse ($phone_numbers as $phone_number)
            <div class="modal" id="delete-confirmation-modal-{{ $phone_number->id }}" tabindex="-1"
                aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-body p-0">
                            <form action="{{ route('phone-numbers.destroy', $phone_number->id) }}" method="POST">
                                @csrf
                                @method('delete')
