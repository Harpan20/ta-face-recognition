<x-layout>
    <x-slot name="head">
        <title>
            {{ __('Edit Phone Number') }} | {{ env('APP_NAME') }}
        </title>
    </x-slot>
    ...
    <div class="content">
        ...
        <div class="mt-5 grid grid-cols-12 gap-6">
            <div class="intro-y col-span-12 lg:col-span-6">
                <!-- BEGIN: Form Layout -->
                <form action="{{ route('phone-numbers.update', $phoneNumber->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="intro-y box p-5">
                            ...
                            <div class="mt-2">
                                <select
                                    ...
                                    @forelse ($countries as $country)
                                        <option value="{{ $country->id }}" @selected(old('country_id', $phoneNumber->country_id) == $country->id)>
                        ...
                        <div class="mt-3">
                            <div class="form-check form-switch">
                                ...
                                <input class="form-check-input {{ $errors->has('is_main') ? '!border-danger' : '' }}"
                                    id="is_main" type="checkbox" name="is_main"
                                    value="{{ old('is_main', $phoneNumber->is_main) }}" @checked(old('is_main') || $phoneNumber->is_main == '1')>
                                <label class="form-check-label" for="is_main">
                                    {{ __('Main Phone Number') }}
                                </label>
                            </div>
                            @error('is_main')
                                <div class="pristine-error mt-2 text-danger">{{ $message }}</div>
                        ...
                        <div class="mt-5 text-right">
                            <a class="btn btn-outline-secondary mr-1 w-24" href="{{ route('phone-numbers.index') }}">
                                {{ __('Cancel') }}
                            </a>
